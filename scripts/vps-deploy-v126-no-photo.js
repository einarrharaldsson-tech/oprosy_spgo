...
const CODE_FILES = [
  ...
  'database/schema.sql',
  'database/migrations/004_drop_response_photo.sql',
  '.env.example',
];
...
const remoteCmd = [
  ...
  'mysql -h "$HOST" -P "${DB_PORT:-3306}" -u "$DB_USER" "$DB_NAME" < database/migrations/004_drop_response_photo.sql || true',
  'mysql -h "$HOST" -P "${DB_PORT:-3306}" -u "$DB_USER" "$DB_NAME" -e "SHOW COLUMNS FROM responses;"',
  'rm -rf server/uploads/photos',
  ...
  'grep -o "1\\.2\\.6" "$KEEP_JS" | head -3 || true',
  'grep -c "photo\\|FrontCamera\\|ResponsePhoto" "$KEEP_JS" || true',
  'pm2 restart oprosy',
  'sleep 1',
...
conn.on('ready', async () => {
  try {
    ...
    for (const f of DELETE_REMOTE) {
      await new Promise((resolve) => sftp.unlink(`${REMOTE}/${f}`, () => resolve()));
      console.log('rm', f);
    }
    ...
    conn.exec(remoteCmd, (e, s) => {
      ...
      s.on('close', (code) => { conn.end(); process.exit(code || 0); });
    });
  } catch (err) {
    console.error(err);
    conn.end();
